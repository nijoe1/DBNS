...
async function main() {
  ...
  let tx = await TokemonInstance.mintCharacter(tokenCharacterNode, {
    gasLimit: 10000000,
    value: 0,
  });
  let reciept = await tx.wait();

  let logsLenght = reciept.logs.length;
  let log = reciept.logs[logsLenght - 2];

  let abiCoder = ethers.utils.defaultAbiCoder;

  let decodedLog = abiCoder.decode(["address", "bytes32", "uint256"], log.data);

  let tokenBountAccount = decodedLog[0];

  console.log(
    "Character minted and tokenBountAccount created at address :",
    decodedLog[0]
  );

  const tokenboundAccountContract = new ethers.Contract(
    tokenBountAccount,
    erc6551AccountAbiV3,
    signer
  );

  const iface = new ethers.utils.Interface(tokemon.abi);

  const encodedMintAssetFunctionData = iface.encodeFunctionData("mintAsset", [
    tokenAssetNode,
  ]);

  console.log(encodedMintAssetFunctionData);

  const executeMintAsset = await tokenboundAccountContract.execute(
    tokemonAddress,
    0,
    encodedMintAssetFunctionData,
    0,
    { gasLimit: 10000000, value: BigInt(0) }
  );

  const receipt = await executeMintAsset.wait();
